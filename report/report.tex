...
\end{itemize}
The dependent variables are the mean, maximum and minimum shepherds' fitness scores as well as the mean corralled and escaped sheep ratios.
...
The chart in Figure ~\ref{fig:ratios_oneSheep} shows that the optimal scenario is two shepherd herding one sheep, since it displays the highest corralled ratio. 
Moreover, adding a third shepherd to the task has a negative effect on the effectiveness, since the escaped ratio rises. 
%\textit{t}-tests were performed to corroborate the statistical difference of the above. We select a threshold of 5\%.
%For the corralled ratio, scenarios with one shepherd and three shepherds produced \textit{p}-values of %TODO compared to two shepherds herding one sheep. 
%Similarly, for the escaped ratio scenarios with two and one shepherds produce \textit{p}-values of %TODO compared to three shepherds herding one sheep

Therefore, we accept the corresponding null hypothesis since there is no evident proportional relationship between the number of shepherds and the corralled ratio. 
However, we can draw a conclusion since the escaped ratio has a proportional relationship with the number of shepherds in the task.
