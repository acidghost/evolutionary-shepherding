...
In order to evaluate the performance of the shepherds we define a performance measure as $c/e$.
This metric is the performance of herding that can be increased by corralling more sheep or allowing less sheep to escape.
...
Table~\ref{tab:1} shows the performance resulting from each simulated run.
The optimal scenario including a single sheep is the version with 2 shepherds, this scenario shows the best performance.  
Increasing the number of shepherds in a single sheep herding task seems to positively affect the performance.
Going from a single shepherd to two shepherds creates a relatively large jump in performance compared to going from 1 to 3 shepherds.
Comparing 2 to 3 shepherds on a single sheep herding task seem to worsen performance.
...
From the results we see performance increasing from a higher number of sheep.
This leads us to state that the $H_3$ is not valid.
...
%ncreasing the number of sheep being herded by a fixed number of shepherds decreases the performance of shepherding.
%no statistical tests were performed due to poor t-test values.
\begin{figure}[ht]
	\centering
...
\end{figure}
%Similarly, the chart in Figure ~\ref{fig:ratios_threeShepherd} shows that the optimal scenario is three shepherds herding two sheep. 
%Since it displays the highes corralled ratio. 
%This time, having only one sheep leads to the highest number of escape ratio. 
%TODO add t/test p values
%3v2 and 3v1 corralled
%3v2 and 3v3 corralled
%3v1 and 3v2 escaped
%3v1 and 3v3 escaped
%Once again, there is no evident proportional relationship between the number of sheep herd and the corralled ratio. Thus we accept the corresponding null hypothesis. 
...
Given heterogeneous controllers, the sheep to shepherd ratio cannot predictably influence the performance shepherds will have in a herding task.
Structural changes to the sheep to shepherd ratio did not evoke similar structural changes to performance.
%Different scenarios proved to be different, with scenarios with two shepherds herding one sheep and three shepherds herding two sheep being optimal for herding. 

Our findings support and expand Potter et al.'s work ~\cite{potter2001heterogeneity}. We show that by increasing task complexity alone, there is no need to implement heterogeneous controllers. Having more adaptable sheep in fact weakens the use of heterogeneity in the scenarios we explored, while varying the sheep to dog ratio is uninformative of the shepherds fitness scores.
