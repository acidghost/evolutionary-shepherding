...
The chart in Figure ~\ref{fig:ratios_oneSheep} shows that the optimal scenario is two shepherd herding one sheep, since it displays the highest corralled ratio with non overlaping confidence intervals compared to one shepherd herding one sheep. 
Since the confidence intervals for one and three shepherd(s) herding one sheep overlap, we perform \textit{t}-test. The test produced \textit{p}-value of 0.005098, which is $\ll0.05$.
Yet, we must accept the corresponding null hypothesis since there is no evident proportional relationship between the number of shepherds and the corralled ratio.
...
Once again, there is no evident proportional relationship between the number of sheep herd and the corralled ratio. Thus we accept the corresponding numm hypothesis. 
