...
Agents have a simple obstacle avoidance system that allows them to not collide each other.
Furthermore, in order to actuate the movements the output from the controller has to be translated into Cartesian coordinates readable by the moving system of the agents.    
These both steps are finally specified in Section~\ref{sec:agents_design}.

\subsection{Herding task and environment}
\label{sec:herding_task_environment}
% herding task + environment
The task domain relates to a herding task that simulates shepherd robots trying to push sheep robots into a corral. 
The environment consists of a $l \times l$ squared pasture with fences on the top and bottom. The corral is positioned on the right side, and the pasture is open on the left side for the sheep to escape.
% Task complexity
...
Thus, we operationalize it as the sheep to shepherd ratio, defined as $\phi = D / S$, where an increased ratio implies a more complex task for the shepherds.
...
Within this system, a coordinate pair is defined as $p = (r, b)$, where $r$ is the distance to the second agent (range) and $b$ is the angle from ???????????????????????????? (bearing). %TODO: fix 
In order to define the position of a group of sheep, we take into account the center of mass $gc$, which is equal to the position of the sheep when there is only one.
%TODO: explain how we calculate the gc
...
This is needed since in different experiment configurations the number of agents varies (see Section~\ref{sec:experiments_performances}).
