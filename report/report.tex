...
Our experiments show that shepherds perform better when assigned homogeneous controllers than heterogeneous controllers. We also find that task complexity is not sufficient to determine the effectiveness heterogeneous controllers have in a herding task.
...
To do so, an agent firstly acquires its inputs and then propagates them through its NN, computing a new target position that the simulation environment will translate in movement to be applied to the agent's virtual body.
...
This is to allows them to effectively herd the sheep by rimming and pushing it towards the corral.
...
Figure~\ref{fig:2v1_homo_vs_hetero} reports the shepherd's fitness score on a scenario with two shepherds and one sheep, while Figure~\ref{fig:3v1_homo_vs_hetero} reports the score on a scenario with three shepherds and one sheep.
...
Relating this to competitive co-evolution, it is reasonable to think that increasing the number of shepherds encourages both shepherds and sheep to pursue their goals more aggressively. %TODO: Really?

%TODO maybe we can introduce a corralled/escaped ratio, making the different setups easy to compare on performance
...
Given heterogeneous controllers and a fixed number of sheep to be herd, adding shepherds encourages more aggressive behaviours. More shepherds cooperate to corral more sheep, but the effectiveness in herding leads to sheep becoming better at escaping. 
