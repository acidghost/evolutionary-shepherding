...
\begin{abstract}
In the context of robotics, intelligent behaviours are often achieved using neural networks as controllers created through evolutionary algorithms.
In the case of multiple individuals within a species, controllers can be either homogeneous or heterogeneous.
In this paper, we simulate a herding task involving robotic shepherds and sheep. 
Through this task, we aim to compare competitive co-evolution of homogeneous and heterogeneous controllers,  and to investigate the effect task complexity has on herding performance.
Our experiments show that shepherds score higher fitness when assigned homogeneous controllers when herding a single intelligent sheep.
Furthermore, given heterogeneous controllers, we find that task complexity is not a predictor of the effectiveness shepherds have in a herding task.
\end{abstract}
...
The evaluation of the individuals' controllers may be done in simulation in order to exploit computers' computational power by evaluating multiple individuals in reasonable time.

% Solvable tasks
Tasks composition and complexity vary based on specific scenarios, making it the subject of many studies aiming to solve different tasks.
...
Moreover, the task complexity can be increased by varying or introducing new agents of existing species (e.g. sherpherds or sheep) or adding new species (e.g. adding a fox).
...
\begin{itemize}
	...
	\item To estimate the effect of homogeneity versus heterogeneity on increasingly difficult tasks.
...
In a herding task, shepherds and sheep have opposing goals.
While the former tries to herd sheep into a corral, the latter tries to escape through the open end of the pasture. 
These species can be competitively co-evolved in a race to evolve strategies to accomplish opposing goals. 
Furthermore, the task complexity can be influenced by varying different parameters such as the number of present agents per species or their relative movement speeds. 
...
\begin{enumerate}
	...
	\item Increasing the number of shepherd herding a fixed number of sheep increases the performance of shepherding.
	\item Increasing the number of sheep being herded by a fixed number of shepherds decreases the performance of shepherding.
\end{enumerate}
...
\begin{enumerate}
	...
	\item Increasing the number of shepherds herding a fixed number of sheep does not affect the performance.
	\item Increasing the number of sheep being herded by a fixed number of shepherds does not affect the performance.
\end{enumerate}
This research will provide a statistical evaluation for $H_1$. Hypothesis $H_2$ and $H_3$ are evaluated through different methods and only show evidences to support or reject them.

\subsection{Contributions}
In this paper we propose a model for running a herding task in a competitive co-evolution framework. 
We have implemented our model and run a thorough set of simulations in order to test the hypothesis detailed in Section~\ref{sec:hypothesis}. 
Hereby we list the main contributions of this paper to the state-of-the-art:
\begin{itemize}
	\item We present fitness functions that reflect the goals for different types of agent (i.e., shepherds, sheep).
	\item We implement and evaluate the effects of heterogeneity within different co-evolution settings.
	...
	\item We propose and motivate a suitable performance measure as the ratio between corralled and escaped sheep.
\end{itemize}

This work sheds a tiny light in the topic of competitive co-evolution by showing its feasibility applied to a herding task accomplished by robots.
...
Moreover, we perform \textit{t}-test on the means of the homogeneous and heterogeneous trials. The test  produced a \textit{p}-value $\ll0.05$ for both cases.
...
\begin{table*}[t]
	\centering
	\begin{tabularx}{\textwidth}{| L{0.6} | L{0.5} | C{1.9} C{1} C{1} | C{1} | C{1} | C{1} |}
		\hline
		\textit{\textbf{Shepherds}} & \textit{\textbf{Sheep}} & \multicolumn{3}{c|}{\textit{\textbf{Fitness}}} & \textbf{\textit{Corralled ratio}} & \textbf{\textit{Escaped ratio}} & \textbf{\textit{Performance}}\\
		& & \textit{Mean} & \textit{Minimum} & \textit{Maximum} & & & \\
		\hline
		1 & 1 & 9884 $\pm$ 28397 & -27922 & 53253 & 0.043 $\pm$ 0.106 & 0.339 $\pm$ 0.274 & 0.126\\
		%\hline
		2 & 1 & 44143 $\pm$ 25778 & -25211 & 71527 & 0.218 $\pm$ 0.251 & 0.367 $\pm$ 0.256 & 0.593\\
		\hline
		3 & 1 & 51618 $\pm$ 19801 & -12015 & 69989 & 0.147 $\pm$ 0.202 & 0.614 $\pm$ 0.206 & 0.239\\
		\hline
		3 & 2 & 62098 $\pm$ 14798 & 564 & 73474 & 0.475 $\pm$ 0.170 & 0.293 $\pm$ 0.192 & 1.618\\
		%\hline
		3 & 3 & 49477 $\pm$ 12561 & 8137 & 63566 & 0.329 $\pm$ 0.162 & 0.493 $\pm$ 0.162 & 0.667\\
		\hline
	\end{tabularx}
	\caption{Performance comparison of the heterogeneous shepherds vs. homogeneous sheep cases, over 40 runs.}
	\label{tab:1}
\end{table*}
...
Here we consider five experiments that relate to the second research question: \textit{How does varying task complexity (by moduling the sheep to shepherds ratio) affect the effectiveness of herding?}. 
...
\end{itemize}
The dependent variables are the number of corralled sheep $c$ and escaped sheep $e$ in the simulation.
In order to evaluate the effectiveness of herding we define a performance measure as $c/e$.
This performance metric can be increased by corralling more sheep or allowing less sheep to escape.

Figure~\ref{fig:ratios_oneSheep} shows the corralled and escaped ratios for different numbers of shepherds herding one sheep, averaged over 40 runs. 
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=1\hsize]{imgs/barchart_vs_one_sheep.png}
	\caption{Corralled and escaped ratios in heterogeneous setting, averaged over 40 runs. Cases with one (red), two (blue) and three (green) shepherds herding one sheep are shown in different bars with 95\% confidence intervals.}
	\label{fig:ratios_oneSheep}
\end{figure}

\begin{figure}[ht]
	\centering
	\includegraphics[width=1\hsize]{imgs/barchart_vs_more_sheep.png}
	\caption{Corralled and escaped ratios in heterogeneous setting, averaged over 40 runs. Cases with one shepherd herding one (red), two (blue) and three (green) sheep are shown in different bars with 95\% confidence intervals.}
	\label{fig:ratios_threeShepherd}
\end{figure}

Table~\ref{tab:1} shows the average performance resulting from each simulated scenario.
For scenarios involving one sheep, the optimal is the version with 2 shepherds, since it shows the best performance.  
The results show that switching from a single shepherd to multiple shepherds in a single sheep herding task seems to positively affect the performance.
The reson behind this might be that multiple shepherds have better performance because cooperation is enhanced as a common goal is pursued.
However, we note that going from a single shepherd to two shepherds leads to a larger increase in performance compared to going from one to three shepherds.
Furthermore, comparing two to three shepherds on a single sheep herding task has a negative effect onperformance.
Therefore we can speculate that $H_2$ is valid, since performance does increase with additional shepherds, although the performance increase is not linear.

%Increasing the number of shepherd herding a fixed number of sheep increases the performance of shepherding.
With regards to scenarios involving three shepherds, the optimal is the version with two sheep since it shows the best performance.
The results show performance increases when switching from a single sheep to multiple sheep being herd by three shepherds.
This leads us to state that the $H_3$ is not valid, since increasing the number of sheep being herded by a fixed number of shepherds does not decrease the performance of shepherding.
This might happen because of the flocking behaviour of scenarios with more then one sheep, making the herding task easier due to cohesion between sheep, preventing scattering.
 

\section{Conclusion}
\label{sec:conclusion}
Firstly, we conclude that shepherds benefit from homogeneous controllers in a competitive co-evolved herding task involving one intelligent sheep.
These findings expand Potter et al.'s work ~\cite{potter2001heterogeneity}. 
We show that having more adaptable sheep in fact weakens the need for heterogeneity in shepherds. 

Secondly, a deeper investigation on heterogeneous controllers shows that the sheep to shepherd ratio cannot predict the performance shepherds will have in a herding task.
Structural changes to the sheep to shepherd ratio (i.e. the specific number of agents of each species in different scenarios) did not evoke corresponding changes to performance.
Finally we note that in both cases, switching from one individual to multiple individuals within a species leads to better performance. 
This is the consequence of cooperative behaviours prefered over individualistic approaches. 
